...
- cnf 트리를 확인하고, set 연산을 적용한다.

### ex10 curve
- Z-order curve(또는 Morton order) : 주어진 2차원 좌표 (x, y)를 비트 인터리빙을 사용해 1차원 값으로 변환한 후, [0, 1] 범위의 실수 값으로 매핑하는 역할
- Z-order curve는 주로 공간 데이터 구조나 2차원 데이터를 1차원으로 정렬하여 저장하거나 처리할 때 사용
- Z-order curve는 다차원 데이터를 1차원으로 변환하면서 공간적 지역성을 최대한 유지하는 방법입니다.
- 비트 인터리빙을 통해 좌표값을 섞어서, 좌표가 가까운 것들이 Z-order 상에서도 가까운 값으로 배치되도록 합니다.
- 주로 공간 데이터베이스, 이미지 처리, 컴퓨터 그래픽스에서 사용됩니다. 예를 들어, 쿼드트리, 힐버트 곡선과 같은 공간 분할 기술에서도 유사한 개념이 사용됩니다.
...
```
1. 비트 인터리빙: x와 y의 비트를 각각 번갈아가며 interleaved라는 변수에 저장합니다. 이는 Z-order 곡선의 특성을 유지합니다.
- 비트 인터리빙은 두 개의 숫자(x와 y)의 비트를 하나씩 교차(번갈아) 배치하여 하나의 값으로 결합하는 과정입니다. 
- 이렇게 결합된 값은 Z-order curve 상에서 두 좌표 간의 상대적인 순서를 유지합니다. 즉, 가까운 좌표는 결합된 값도 가까운 값을 가지게 됩니다.

2. 정규화: 최종적으로 interleaved 값을 2^32 - 1로 나누어, 64비트 정수를 0과 1 사이의 실수로 변환합니다.
